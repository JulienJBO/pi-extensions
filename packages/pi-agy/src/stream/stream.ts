...
import { AntigravityAccountPool, type AntigravityAccount } from "../accounts/pool.js";
...
async function requestWithAccount(
  account: AntigravityAccount,
  model: Model<Api>,
  context: Context,
  opts: AntigravityStreamOptions,
): Promise<AccountRequestResult> {
  const creds = parseApiKey(account.apiKey);
  ...
  const pinnedRuntimeModel = antigravityEnv("RUNTIME_MODEL")?.trim();
  const baseRuntimeModel = pinnedRuntimeModel || getAntigravityRequestModelId(model.id, effort);

  let initialRuntimeModel = baseRuntimeModel;
  if (!isKnownModel && !pinnedRuntimeModel) {
  ...
  for (let candIdx = 0; candIdx < runtimeCandidates.length; candIdx++) {
    ...
    if (response?.status === 404) {
      if (candIdx + 1 < runtimeCandidates.length) continue;
  ...
  return { response, lastText, projectId, runtimeModel };
}

function resetOutput(output: AssistantMessage): void {
  output.content = [];
  output.usage = {
    input: 0,
    output: 0,
...
async function writeDebugFailure(
  model: Model<Api>,
  context: Context,
  opts: AntigravityStreamOptions,
  result: AccountRequestResult,
): Promise<void> {
  if (antigravityEnv("DEBUG_DUMP") !== "1") return;
  try {
    const { writeFileSync } = await import("node:fs");
    const body = JSON.stringify(
      buildRequest(model, context, result.projectId, opts, result.runtimeModel),
    );
      ...
      JSON.stringify(
        {
          ...
          lastText: result.lastText.slice(0, 4000),
          body: parsedBody,
        },
        null,
        2,
    ...
    // Ignore diagnostic dump failures.
  }
}

function requestFailure(result: AccountRequestResult): Error {
  const status = result.response?.status;
  const friendly = friendlyAntigravityError(status, result.lastText);
  if (status === 429 && /(?:Quota reached|Rate limited)/i.test(friendly)) {
    return new Error(friendly);
  }
  return new Error(
    `Antigravity API error (${status ?? "no response"}, ${formatRequestDiagnostics({ projectId: result.projectId, runtimeModel: result.runtimeModel })}): ${friendly}`,
  );
}

export function createAntigravityStream(
  accountPool = new AntigravityAccountPool(),
): (
  model: Model<Api>,
  context: Context,
  options?: AntigravityStreamOptions,
) => AssistantMessageEventStream {
  return function pooledAntigravityStream(
    model: Model<Api>,
    context: Context,
    options?: AntigravityStreamOptions,
  ): AssistantMessageEventStream {
    ...
    void runWithDiagnostics(async () => {
      ...
      try {
        ...
        for (let emptyAttempt = 0; emptyAttempt <= 2; emptyAttempt++) {
          ...
          const accounts = await accountPool.resolve(opts.apiKey);
          ...
          for (const [accountIndex, account] of accounts.entries()) {
            const result = await requestWithAccount(account, model, context, opts);
            if (!result.response?.ok) {
              await writeDebugFailure(model, context, opts, result);
              if (result.response?.status === 429) {
                accountPool.markRateLimited(account, accounts);
                if (accountIndex + 1 < accounts.length) continue;
              }
              throw requestFailure(result);
            }

            accountPool.markSuccessful(account);
            resetOutput(output);
            received = await streamResponse(result.response, stream, output);
...
export const streamAntigravity = createAntigravityStream();
