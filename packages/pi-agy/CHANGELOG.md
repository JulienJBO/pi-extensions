...
## [0.6.0-multi.0] - Unreleased

### Added

- **Multi-account pool:** Register ten independent OAuth slots (`antigravity`, then `antigravity-2` through `antigravity-10`) and rotate through every linked account after pre-stream HTTP 429 responses.
- **Sticky preference:** Keep using the last successful account instead of returning to account 1 after every turn.

## [0.5.0-multi.0] - 2026-08-26

### Added

- **Two-account failover:** Link a second Google account and retry an untouched request when account 1 returns a pre-stream HTTP 429.
- **Account diagnostics:** `/antigravity.accounts` reports linked slots and the active preference without exposing credentials.
- **Credential isolation:** Both accounts remain in Pi's normal provider-scoped OAuth store; the failover pool does not persist or copy tokens.
